...
class AngoraSubtitle {
	...
	public static function init( ) {
		...
		if ( $post !== null and get_post_meta( $post->ID, '_wp_page_template', true ) != 'templates/front.php' ) {
			if ( class_exists( 'AngoraAdmin' ) ) {
				AngoraAdmin::addMetaBox('subtitle');
			}
		}
	}
	...
	public static function content( $post ) {
		...
		<div class="meta-pt-15">
			...
			<p>' . esc_html__( 'Example', 'angora' ) . ', <strong>' . esc_html__( 'Lorem ipsum dolor sit amet.', 'angora' ) . '</strong></p>
		</div>';
...
if ( class_exists( 'AngoraAdmin' ) ) {
	AngoraAdmin::addAction('subtitle');
}

add_action( 'save_post', array( 'AngoraSubtitle', 'save' ) );
