function angora_filter_params( ) {
	jQuery( '[data-header-section]' ).hide( );
	jQuery( '[data-header-section="' + jQuery( '#header-section' ).val( ) + '"]' ).show( );
}

function angora_slides_content( ) {
	if ( angoraSlidesContent.length > 0 ) {
		jQuery.each( angoraSlidesContent, function( that, node ) {
...
jQuery( document ).ready( function( jQuery ) {
	jQuery( '.meta-item-upload' ).on("click", function(e) {
		e.preventDefault( );
		...
		frame = wp.media( {
			title: angora_home_lng.insert_media,
			frame: 'post',
			multiple: is_multiple,
			library: { type: 'image' },
			button: { text: angora_home_lng.insert_media },
		} );
			...
			if ( ! is_multiple ) jQuery( area ).val( imageArray.join( ', ' ) );
				...
				if ( area_id == '#slideshow-fields' ) {
					...
					images.each( function( image ) {
						...
						'  ' + angora_home_lng.image + ' <span class="meta-item-row-alt-c">-</span>' +
						'  <input type="text" class="meta-item-l alt" name="slideshow-alt-images[]" value="' + image.attributes.url + '">' +
						'  <input type="button" class="button" data-remove-image="true" value="' + angora_home_lng.remove + '">' +
						'</div>';
	...
	jQuery( '#header-section' ).on( 'change', function( ) {
		angora_filter_params( );
	} );

	angora_filter_params( );
	angora_slides_content( );
} );
