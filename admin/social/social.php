        ...
        public function render() {
            ...
            if (isset($this->value) && is_array($this->value)) {
                ...
                foreach ($socials as $social) {
							...
							<fieldset class="redux-field" data-id="' . esc_attr( $this->field['id'] ) . '">
								...
								<div>
                    				<ul id="' . esc_attr( $this->field['id'] ) . '-ul" class="redux-list">
                    					<li>
											<input type="text" id="' . esc_attr( $this->field['id'] . '-title_' . $x ) . '" name="' . esc_attr( $this->field['name'] . '[' . $x ) . '][title]" value="' . esc_attr( $social['title'] ) . '" placeholder="' . esc_attr__('Label', 'angora') . '" class="full-text social-title" />
										</li>
                    					<li>
											<input type="text" id="' . esc_attr( $this->field['id'] . '-url_' . $x ) . '" name="' . esc_attr( $this->field['name'] . '[' . $x ) . '][url]" value="' . esc_attr( $social['url'] ) . '" class="full-text" placeholder="' . esc_attr__('Link', 'angora') . '" />
										</li>
                    					<li>
					...
					if ( isset( $this->field['options'] ) && !empty( $this->field['options'] ) ) {
                        $placeholder = (isset($this->field['placeholder']['options'])) ? $this->field['placeholder']['options'] : esc_html__( 'Select an Icon', 'angora' );
                    ...
                    echo '					<li>
												<a href="javascript:void(0);" class="button deletion redux-remove">' . esc_html__('Delete Link', 'angora') . '</a>
											</li>
                    					</ul>
									</div>
								</fieldset>
            ...
            if ($x == 0) {
                echo '<div class="redux-accordion-group hidden">
						<fieldset class="redux-field" data-id="' . esc_attr( $this->field['id'] ) . '">
							<h3>
								<span class="redux-header">' . esc_html__('New Social', 'angora') . '</span>
							</h3>
						  	<div>
								<ul id="' . esc_attr( $this->field['id'] ) . '-ul" class="redux-list">';
                
				$placeholder = ( isset( $this->field['placeholder']['title'] ) ) ? $this->field['placeholder']['title'] : esc_html__( 'Label', 'angora' );
				...
				$placeholder = ( isset( $this->field['placeholder']['url'] ) ) ? $this->field['placeholder']['url'] : esc_html__( 'Link', 'angora' );
				...
				if ( isset( $this->field['options'] ) && !empty( $this->field['options'] ) ) {
					$placeholder = ( isset( $this->field['placeholder']['select'] ) ) ? $this->field['placeholder']['select'] : esc_html__( 'Select an Icon', 'angora' );
            ...
            echo '</div>
				  <a href="javascript:void(0);" class="button redux-add button-primary" rel-id="' . esc_attr( $this->field['id'] ) . '-ul" rel-name="' . esc_attr( $this->field['name'] ) . '[title][]">' . esc_html__('Add Link', 'angora') . '</a>
				  <br/>';
		}
