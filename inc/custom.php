<?php
// Breadcrumbs
if ( ! function_exists( 'angora_breadcrumbs' ) ) {
	function angora_breadcrumbs( ) {
		global $post, $angoraConfig;

		$text['home']     = ( ! empty( $angoraConfig['home-page-title'] ) ? $angoraConfig['home-page-title'] : esc_html__( 'Home', 'angora' ) );
		$text['category'] = '%s';
		$text['search']   = esc_html__( 'Search Results for Query &ndash; %s', 'angora' );
		$text['tag']      = esc_html__( 'Posts Tagged &ndash; %s', 'angora' );
		$text['author']   = esc_html__( 'Articles Posted by %s', 'angora' );
		$text['404']      = esc_html__( 'Error 404', 'angora' );
		$text['page']     = esc_html__( 'Page', 'angora' );
